...
const Experiences = () => {
    ...
    const onFinishHander = async (values) => {
        ...
        try {
            ...
            if (selectedItemForEdit) {
                response = await axios.post('/api/v1/update-experience', { _id: selectedItemForEdit._id, ...values })
            } else {
                response = await axios.post('/api/v1/add-experience', values)
            }
    ...
    const onDelete = async (item) => {
        try {
            dispatch(showLoading())
            const response = await axios.post('/api/v1/delete-experience', { _id: item._id })
            ...
            <div className="grid grid-cols-4 sm:grid-cols-2 mob:grid-cols-1 gap-5 ">
                {experience.map((exp, i) => (
                    <div key={i} className="shadow border border-gray-300 p-5 rounded">
                        <h1 className='text-xl text-secondary'>{exp.period}</h1>
                        <h1 >Company : <span className='text-lg'>{exp.company}</span></h1>
                        <h1>Role : <span>{exp.title}</span></h1>
...

            {
                (type === 'add' || selectedItemForEdit) && <Modal footer={null} onCancel={() => {
                    ...
                    <Form layout="vertical" onFinish={onFinishHander} initialValues={selectedItemForEdit}>
                        ...
                        <Form.Item name="reference" label="Company Reference">
                            <input type="text" placeholder='Company Reference' />
                        </Form.Item>
